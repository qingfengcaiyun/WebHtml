(function ($) {
    var bigColorPicker = new function () {
        ...
        this.init = function () {
            // 计算第一列的数组
            var mixColorArray = [];
            var blackWhiteGradientArray = gradientColor(new RGB(0, 0, 0),
                    new RGB(255, 255, 255));
            for (var i = 0; i < blackWhiteGradientArray.length; i++) {
                mixColorArray[i] = blackWhiteGradientArray[i];
            }
            mixColorArray.push(new RGB(255, 0, 0), new RGB(0, 255, 0), new RGB(
                    0, 0, 255), new RGB(255, 255, 0), new RGB(0, 255, 255),
                    new RGB(255, 0, 255));
            ...
            cornerColorArray.push(generateBlockcornerColor(0),
                    generateBlockcornerColor(51),
                    generateBlockcornerColor(102),
                    generateBlockcornerColor(153),
                    generateBlockcornerColor(204),
            ...
            for (var i = 0; i < halfOfAllArray1.length; i++) {
                allColorArray[i + 2] = halfOfAllArray1[i]
                        .concat(halfOfAllArray2[i]);
            }
        ...
        * 字符串：把选择的颜色设置到id为"字符串"的元素的value上。 函数：执行传入的函数以实现自定义的获取颜色后的动作。
        * engine:可以是undefined、p(或P)、l(或L)， 在bigColorpicker展现颜色选择区域小格时有两种实现方式：
        * 一、是一张背景图片，采用光标定位的方式获取颜色。p(或P) 二、每个小格就是一个li，设置li的背景颜色。l(或L)
        * 实现方式的不同，效率有所差异，可以自己在使用时选择，默认是p(或P)。 undefined（不传参数时）：使用默认p(或P)
        */
        this.showPicker = function (callback, engine) {
            bigColorPicker.targetEl = this;
            $(bigColorPicker.targetEl).data("bigpickerCallback",
            ...
            if ($("#bigpicker").length <= 0) {
                $(document.body)
                        .append(
                                '<div id="bigpicker" class="bigpicker"><ul class="bigpicker-bgview-text" ><li><div id="bigBgshowDiv"></div></li><li><input id="bigHexColorText" size="7" maxlength="7" value="#000000" /></li></ul><div id="bigSections" class="bigpicker-sections-color"></div><div id="bigLayout" class="biglayout" ></div></div>');
            }
            ...
            // 颜色拾取器text输入框注册事件
            $("#bigHexColorText").unbind("keypress").unbind("keyup").unbind(
            ...
            if (engine != undefined && engine == "L") {
                ...
                $("#bigSections").unbind("mousemove").unbind("mouseout")
                        .unbind("click").removeClass("bigpicker-bgimage");
                ;
                generateBgImage();
            } else {
                ...
                $("#bigSections")
                        ...
                        .mousemove(
                                function (event) {
                                    var $this = $(this);
                                    var cursorXPos = event.pageX
                                            - $this.offset().left;
                                    var cursorYPos = event.pageY
                                            - $this.offset().top;
                                    var xi = 0;
                                    var yi = 0;
                                    for (var i = 0; i < (sideLength * 3 + 2); i++) {
                                        if (cursorXPos >= xSections[i][0]
                                                && cursorXPos <= xSections[i][1]) {
                                            xi = i;
                                            break;
                                        }
                                    }
                                    for (var i = 0; i < (sideLength * 3); i++) {
                                        if (cursorYPos >= ySections[i][0]
                                                && cursorYPos <= ySections[i][1]) {
                                            yi = i;
                                            break;
                                        }
                                    }
                                    ...
                                    $("#bigBgshowDiv").css("backgroundColor",
                                            hex);
        ...
        function bigpickerHide(event) {
            ...
            if (cursorX < p.left || cursorX > p.left + $bigpicker.width()
                    || cursorY < p.top || cursorY > p.top + $bigpicker.height()) {
                flag = false;
            }
            if (!flag) {
                $("#bigpicker").hide();
        ...
        function calculatePosition($el) {
            ...
            var w = compatMode ? document.documentElement.clientWidth
                    : document.body.clientWidth;
            var h = compatMode ? document.documentElement.clientHeight
        ...
        function invokeCallBack(hex) {
            var callback_ = $(bigColorPicker.targetEl)
                    .data("bigpickerCallback");
        ...
        function gradientColor(startColor, endColor) {
            ...
            for (var i = 1; i < sideLength - 1; i++) {
                gradientArray[i] = new RGB(startColor.r + i * averageR,
                        startColor.g + i * averageG, startColor.b + i
                                * averageB);
            }
            return gradientArray;
        }
        /*
        * 获取一个二维区间数组比如[0,11],[12,23],[24,37].. sl:区间的长度
        */
        ...
        function generateBgImage() {
            ...
            for (var i = 0; i < sideLength * 3 + 2; i++) {
                ...
                for (var j = 0; j < sideLength * 2; j++) {
                    ulArray.push("<li data-color='" + allColorArray[i][j]
                            + "' style='background-color: "
                            + allColorArray[i][j] + ";' ></li>");
                }
                ulArray.push("</ul>");
            }
